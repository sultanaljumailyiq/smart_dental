...
const InteractiveCalendar: React.FC<InteractiveCalendarProps> = ({
  ...
  const onlineBookings = appointments.filter(apt => apt.source === "online_booking").length;
  const pendingApprovals = appointments.filter(apt => apt.source === "online_booking" && apt.status === "pending").length;
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-3">
                <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl">
                  <div className="text-xs text-indigo-100">إجمالي المواعيد</div>
                  <div className="text-lg font-bold">{totalAppointments}</div>
                </div>
                {onlineBookings > 0 && (
                  <div className="bg-purple-500/30 backdrop-blur-sm px-4 py-2 rounded-xl">
                    <div className="text-xs text-purple-100">حجوزات إلكترونية</div>
                    <div className="text-lg font-bold">{onlineBookings}</div>
                  </div>
                )}
                {pendingApprovals > 0 && (
                  <div className="bg-amber-500/30 backdrop-blur-sm px-4 py-2 rounded-xl animate-pulse">
                    <div className="text-xs text-amber-100">بانتظار الموافقة</div>
                    <div className="text-lg font-bold">{pendingApprovals}</div>
                  </div>
                )}
              </div>
            </div>
              ...
              {selectedDateAppointments.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all mx-auto"
                  >
                    <Plus className="w-4 h-4" />
                    إضافة موعد جديد
                  </button>
