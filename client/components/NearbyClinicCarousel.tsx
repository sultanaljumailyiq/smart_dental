...
import { MapPin, Star, Phone, Navigation, Clock, Shield, Search } from "lucide-react";
...
export function NearbyClinicCarousel({
  ...
  // Fetch clinics when geolocation is obtained
  useEffect(() => {
  ...
  const getUserLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.error('Geolocation error, using fallback mode:', error);
          // Fallback: fetch promoted clinics if geolocation fails
          fetchClinics();
        },
        { timeout: 5000 }
      );
    } else {
      // No geolocation support: fetch without coordinates
      fetchClinics();
    }
  };
  ...
  const fetchClinics = async (lat?: number, lng?: number) => {
    try {
      ...
      // Use provided coordinates or props
      const finalLat = lat ?? userLat;
      const finalLng = lng ?? userLng;
      
      if (finalLat && finalLng) {
        params.append('userLat', finalLat.toString());
        params.append('userLng', finalLng.toString());
        params.append('radiusKm', '50');
        params.append('mode', mode === 'distance' ? 'distance' : 'promoted');
      } else {
        // No coordinates: use promoted mode
        params.append('mode', 'promoted');
      }
      ...
      const url = `/api/clinics?${params.toString()}`;
          ...
          <div className="flex-1 relative">
            {!API_KEY ? (
              ...
              <APIProvider apiKey={API_KEY}>
                <Map
                  mapId="nearby-clinic-carousel-map"
